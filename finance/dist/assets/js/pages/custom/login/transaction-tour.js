$(document).ready(function () {
	...
	$('#btn_excel').on('click', function (e) {
		...
		$.ajax({
			type: "POST",
			url: `${HOST_URL}/finance/report/export_data_csv_transaction_tour_all`,
			dataType: "JSON",
			data: {
	...
	$('#btn_pdf').on('click', function (e) {
		...
		$.ajax({
			type: "POST",
			url: `${HOST_URL}/finance/report/print_data_pdf_transaction_tour_all`,
			data: {
				data_check: rows_selected.join(","),
				date_range: date_range
			},
	...
	$("#btnInputKredit").on("click", function () {
		...
		if (nominal != null && nominal >= 5000 && nominal != "" && nis != null && nis != "" && tahun_ajaran != null && tahun_ajaran != "" && tanggal_transaksi != null && tanggal_transaksi != "" && jenis_tabungan != null && jenis_tabungan != "" && tingkat != null && tingkat != "") {

			if (stat_close == true) {

				Swal.fire({
					title: "Peringatan!",
					html: "Apakah anda yakin ingin Menyetor Tabungan Wisata atas nama <b>" + nama.toUpperCase() + "</b> dengan Nominal Rp." + nominal + " ?",
					icon: "warning",
					...
					if (result.value) {
						...
						Swal.fire("Dibatalkan!", "Setoran Tabungan Wisata atas nama <b>" + nama.toUpperCase() + "</b> batal diinputkan.", "error");
						return false;
					}
				});
				...
				if (nama_nasabah != null && nama_nasabah != "" && tingkat != null && tingkat != "" && jenis_tabungan != null && jenis_tabungan != "") {
					...
					Swal.fire({
						title: "Peringatan!",
						html: "Apakah anda yakin ingin MENYETOR TABUNGAN WISATA & MENAMBAH NASABAH BARU atas nama <b>" + nama_nasabah.toUpperCase() + " (" + nis + ")</b> dengan Nominal Rp." + nominal + " ?",
						icon: "warning",
						showCancelButton: true,
						confirmButtonColor: "#DD6B55",
						confirmButtonText: "Ya, Setor!",
						...
						if (result.value) {
							...
							$.ajax({
								type: "POST",
								url: `${HOST_URL}/finance/savings/post_tour_credit_new_client`,
								dataType: "JSON",
								data: {
									nis: nis,
									nominal: nominal,
							...
							Swal.fire("Dibatalkan!", "Setoran Tabungan Wsiata & Tambah Nasabah Baru atas nama <b>" + nama.toUpperCase() + " (" + nis + ")</b> batal diinputkan.", "error");
							return false;
						}
					});
				} else {
			...
			Swal.fire({
				html: "Opps!, Pastikan Semua Inputan Terisi dengan Benar & Nominal Tidak Boleh < 5000, Silahkan input ulang.",
				icon: "error",
				buttonsStyling: false,
				confirmButtonText: "Oke!",
				customClass: {
	...
	$("#btnUpdateKredit").on("click", function () {
		...
		if (nominal != null && nominal >= 5000 && nominal != "" && nis != null && nis != "" && tahun_ajaran != null && tahun_ajaran != "" && tanggal_transaksi != null && tanggal_transaksi != "" && tingkat != null && tingkat != "") {

			Swal.fire({
				title: "Peringatan!",
				html: "Apakah anda yakin Update Kredit Tabungan Wisata atas nama <b>" + nama.toUpperCase() + "</b> dengan Nominal Rp." + nominal + " ?",
				icon: "warning",
				showCancelButton: true,
				confirmButtonColor: "#DD6B55",
				confirmButtonText: "Ya, Setor!",
				...
				if (result.value) {
					...
					Swal.fire("Dibatalkan!", "Edit Setoran Tabungan Wisata atas nama <b>" + nama.toUpperCase() + "</b> batal diubah.", "error");
					return false;
				}
			});
			...
			Swal.fire({
				html: "Opps!, Pastikan Inputan Terisi dengan Benar & Nominal Tidak Boleh < 5000 dan Kosong, Silahkan input ulang.",
				icon: "error",
				buttonsStyling: false,
				confirmButtonText: "Oke!",
				customClass: {
	...
	$("#btnInputDebet").on("click", function () {
		...
		if (nominal <= saldo) {
			...
			if (nominal != null && nominal >= 5000 && nominal != "" && nis != null && nis != "" && tahun_ajaran != null && tahun_ajaran != "" && tanggal_transaksi != null && tanggal_transaksi != "" && tingkat != null && tingkat != "") {
				...
				Swal.fire({
					title: "Peringatan!",
					html: "Apakah anda yakin Menarik Tabungan Wisata atas nama <b>" + nama.toUpperCase() + "</b> dengan Nominal Rp." + nominal + " ?",
					icon: "warning",
					showCancelButton: true,
					confirmButtonColor: "#DD6B55",
					...
					if (result.value) {
						...
						Swal.fire("Dibatalkan!", "Penarikan Tabungan Wisata atas nama <b>" + nama.toUpperCase() + "</b> batal diubah.", "error");
						return false;
					}
				});
	...
	$("#btnUpdateDebet").on("click", function () {
		...
		if (nominal <= saldo) {
			...
			if (nominal != null && nominal >= 5000 && nominal != "" && nis != null && nis != "" && tahun_ajaran != null && tahun_ajaran != "" && tanggal_transaksi != null && tanggal_transaksi != "" && tingkat != null && tingkat != "") {
				...
				Swal.fire({
					title: "Peringatan!",
					html: "Apakah anda yakin Update Penarikan Tabungan Wisata atas nama <b>" + nama.toUpperCase() + "</b> dengan Nominal Rp." + nominal + " ?",
					icon: "warning",
					showCancelButton: true,
					confirmButtonColor: "#DD6B55",
					confirmButtonText: "Ya, Tarik!",
					...
					if (result.value) {
						...
						Swal.fire("Dibatalkan!", "Edit Penarikan Tabungan Wisata atas nama <b>" + nama.toUpperCase() + "</b> batal diubah.", "error");
						return false;
					}
				});
	...
	function show_transaksi() {
		...
		$.ajax({
			type: "GET",
			url: `${HOST_URL}/finance/savings/get_all_tour_transaction`,
			async: false,
			data: {
				start_date: lstart,
				end_date: lend,
					...
					if (data[i].status_edit == 1) {
						...
						if (data[i].status_kredit_debet == "1") {
							var option = "<div class='dropdown dropdown-inline'>" +
								...
								data[i].id_transaksi_wisata + "' data-nis_siswa='" + data[i].nis_siswa + "' data-nama_lengkap='" + data[i].nama_lengkap + "' data-id_tingkat='" + data[i].id_tingkat + "' data-nomor_transaksi='" + data[i].nomor_transaksi_wisata +
	...
	$("#tb_transaksi").on("click", ".delete_transaksi_kredit", function () {
		...
		Swal.fire({
			...
			closeOnConfirm: false,
			closeOnCancel: false,
			html: "Apakah anda yakin ingin menghapus Transaksi Wisata <b>" + nomor_transaksi + "</b>  (" + jenis_transaksi + ") atas nama <b>'" +
			...
			if (result.value) {
				...
				Swal.fire("Dibatalkan!", "Penghapusan Transaksi Wisata <b>" + nomor_transaksi + "</b>  (" + jenis_transaksi + ") atas nama <b>'" + nama_siswa.toUpperCase() + "'</b> (" + nis_siswa + ") batal dihapus.", "error");

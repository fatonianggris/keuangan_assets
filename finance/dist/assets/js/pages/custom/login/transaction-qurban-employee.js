$(document).ready(function () {
	...
	$('#btn_excel').on('click', function (e) {
		...
		$.ajax({
			type: "POST",
			url: `${HOST_URL}/finance/report/export_data_csv_transaction_qurban_all_employee`,
			dataType: "JSON",
			data: {
	...
	$('#btn_pdf').on('click', function (e) {
		...
		$.ajax({
			type: "POST",
			url: `${HOST_URL}/finance/report/print_data_pdf_transaction_qurban_all_employee`,
			data: {
				data_check: rows_selected.join(","),
				date_range: date_range
			},
		...
		if (nominal != null && nominal >= 1000 && nominal != "" && nip != null && nip != "" && tahun_ajaran != null && tahun_ajaran != "" && tanggal_transaksi != null && tanggal_transaksi != "" && jenis_tabungan != null && jenis_tabungan != "" && tingkat != null && tingkat != "") {
			if (window.bundleObj.getOTPKredit() === false) {
				...
				if (stat_close == true) {
					...
					Swal.fire({
						title: "Peringatan!",
						html: "Apakah anda yakin ingin Menyetor Tabungan Qurban Pegawai atas nama <b>" + nama.toUpperCase() + "</b> dengan Nominal Rp." + nominal + " ?",
						icon: "warning",
						...
						if (result.value) {
							...
							Swal.fire("Dibatalkan!", "Setoran Tabungan Qurban Pegawai atas nama <b>" + nama.toUpperCase() + "</b> batal diinputkan.", "error");
							return false;
						}
					});
					...
					if (nama_nasabah != null && nama_nasabah != "" && status_pegawai != null && status_pegawai != "" && tingkat != null && tingkat != "" && jenis_tabungan != null && jenis_tabungan != "") {

						Swal.fire({
							title: "Peringatan!",
							html: "Apakah anda yakin ingin MENYETOR TABUNGAN QURBAN & MENAMBAH NASABAH PEGAWAI BARU atas nama <b>" + nama_nasabah.toUpperCase() + " (" + nip + ")</b> dengan Nominal Rp." + nominal + " ?",
							icon: "warning",
							showCancelButton: true,
							confirmButtonColor: "#DD6B55",
							confirmButtonText: "Ya, Setor!",
							...
							if (result.value) {
								...
								$.ajax({
									type: "POST",
									url: `${HOST_URL}/finance/savings/post_qurban_credit_new_client_employee`,
									dataType: "JSON",
									data: {
										nip: nip,
										nominal: nominal,
								...
								Swal.fire("Dibatalkan!", "Setoran Tabungan Qurban & Tambah Nasabah Pegawai Baru atas nama <b>" + nama_nasabah.toUpperCase() + " (" + nip + ")</b> batal diinputkan.", "error");
								return false;
							}
						});
					} else {
			...
			Swal.fire({
				html: "Opps!, Pastikan Semua Inputan Terisi dengan Benar & Nominal Tidak Boleh < 1000, Silahkan input ulang.",
				icon: "error",
				buttonsStyling: false,
				confirmButtonText: "Oke!",
				customClass: {
	...
	$("#btnUpdateKredit").on("click", function () {
		...
		if (nominal != null && nominal >= 1000 && nominal != "" && nip != null && nip != "" && tahun_ajaran != null && tahun_ajaran != "" && tanggal_transaksi != null && tanggal_transaksi != "" && tingkat != null && tingkat != "") {
			if (window.bundleObj.getOTPKreditEdit() === false) {
				...
				Swal.fire({
					title: "Peringatan!",
					html: "Apakah anda yakin Update Kredit Tabungan Qurban Pegawai atas nama <b>" + nama.toUpperCase() + "</b> dengan Nominal Rp." + nominal + " ?",
					icon: "warning",
					showCancelButton: true,
					confirmButtonColor: "#DD6B55",
					confirmButtonText: "Ya, Setor!",
					...
					if (result.value) {
						...
						Swal.fire("Dibatalkan!", "Edit Setoran Tabungan Qurban Pegawai atas nama <b>" + nama.toUpperCase() + "</b> batal diubah.", "error");
						return false;
					}
				});
		...
		if (nominal <= saldo) {
			...
			if (nominal != null && nominal >= 1000 && nominal != "" && nip != null && nip != "" && tahun_ajaran != null && tahun_ajaran != "" && tanggal_transaksi != null && tanggal_transaksi != "" && tingkat != null && tingkat != "") {
				if (window.bundleObj.getOTPDebet() === false) {
					...
					Swal.fire({
						title: "Peringatan!",
						html: "Apakah anda yakin Menarik Tabungan Qurban Pegawai atas nama <b>" + nama.toUpperCase() + "</b> dengan Nominal Rp." + nominal + " ?",
						icon: "warning",
						showCancelButton: true,
						confirmButtonColor: "#DD6B55",
						...
						if (result.value) {
							...
							Swal.fire("Dibatalkan!", "Penarikan Tabungan Qurban Pegawai atas nama <b>" + nama.toUpperCase() + "</b> batal diubah.", "error");
							return false;
						}
					});
		...
		if (nominal <= saldo) {
			...
			if (nominal != null && nominal >= 1000 && nominal != "" && nip != null && nip != "" && tahun_ajaran != null && tahun_ajaran != "" && tanggal_transaksi != null && tanggal_transaksi != "" && tingkat != null && tingkat != "") {
				...
				if (window.bundleObj.getOTPDebetEdit() === false) {
					...
					Swal.fire({
						title: "Peringatan!",
						html: "Apakah anda yakin Update Penarikan Tabungan Qurban Pegawai atas nama <b>" + nama.toUpperCase() + "</b> dengan Nominal Rp." + nominal + " ?",
						icon: "warning",
						showCancelButton: true,
						confirmButtonColor: "#DD6B55",
						confirmButtonText: "Ya, Tarik!",
						...
						if (result.value) {
							...
							Swal.fire("Dibatalkan!", "Edit Penarikan Tabungan Qurban Pegawai atas nama <b>" + nama.toUpperCase() + "</b> batal diubah.", "error");
							return false;
						}
					});
	...
	function show_transaksi() {
		...
		$.ajax({
			type: "GET",
			url: `${HOST_URL}/finance/savings/get_all_qurban_transaction_employee`,
			async: false,
			data: {
				start_date: lstart,
				end_date: lend,
			...
			success: function (data) {
				...
				for (var i = 0; i < data.length; i++) {
					...
					if (data[i].status_edit == 1) {
						...
						if (data[i].status_kredit_debet == "1") {
							var option = "<div class='dropdown dropdown-inline'>" +
								...
								data[i].id_transaksi_qurban_pegawai + "' data-nip_pegawai='" + data[i].nip_pegawai + "' data-nama_lengkap='" + data[i].nama_lengkap + "' data-id_tingkat='" + data[i].id_tingkat + "' data-nomor_transaksi='" + data[i].nomor_transaksi_qurban +
								...
								"' data-nip_pegawai='" + data[i].nip_pegawai + "' data-nomor_transaksi='" + data[i].nomor_transaksi_qurban + "' data-nama_lengkap='" + data[i].nama_lengkap + "' data-tingkat='" + nama_tingkat +
					...
					html +=
						"<tr class='" + `${bg_color}` + "'>" +
						"<td>" +
						`${data[i].id_transaksi_qurban_pegawai}` +
						"</td>" +
	...
	$("#tb_transaksi").on("click", ".delete_transaksi_kredit", function () {
		...
		Swal.fire({
			...
			closeOnConfirm: false,
			closeOnCancel: true,
			html: "Apakah anda yakin ingin menghapus Transaksi Qurban Pegawai <b>" + nomor_transaksi + "</b> (" + jenis_transaksi + ") atas nama <b>'" +
			...
			if (result.value) {
				...
				Swal.fire("Dibatalkan!", "Penghapusan Transaksi Qurban Pegawai <b>" + nomor_transaksi + "</b> (" + jenis_transaksi + ") atas nama <b>'" + nama_pegawai.toUpperCase() + "'</b> (" + nip_pegawai + ") batal dihapus.", "error");
				return false;
			}
		});
